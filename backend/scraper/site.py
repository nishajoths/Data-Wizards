...
def scrape_website(url, extract_product_info=False, search_keywords=None):
    ...
    try:
        ...
        # Return error information with enhanced error detail
        end_time = time.time()
        error_detail = str(e)
        ...
        return {
            ...
            "content": {"text_content": [], "images": [], "image_texts": []},
            "extracted_at": time.time()
        }
...
def store_in_mongodb(scraped_data):
    ...
    if "_id" not in scraped_data:
        scraped_data["_id"] = ObjectId()
    
    # Store the data
    db.sites.update_one({"url": scraped_data["url"]}, {"$set": scraped_data}, upsert=True)
    
    return scraped_data["_id"]
